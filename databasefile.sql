...
select e.employeeid,concat_ws(' ',e.Lastname,e.Firstname) as 'Full Name',sum(od.quantity*od.unitprice*(1 - od.Discount)) as 'total sales'
from employees e 
...
select c.customername,sum(od.quantity*od.unitprice*(1 - od.Discount)) as 'total amount spent ($)'
from customers c 
...
select monthname(o.orderdate) month,sum(od.quantity*od.unitprice*(1 - od.Discount)) as 'total monthly sales ($)'
from orders o
left join orderdetails od on o.orderid = od.orderid
...
SalesGrowth AS (
    SELECT sp1.ProductID,sp1.Year,sp1.TotalSales AS CurrentYearSales,sp2.TotalSales AS PreviousYearSales,
        CASE 
            WHEN sp2.TotalSales IS NULL THEN NULL
            ELSE ((sp1.TotalSales - sp2.TotalSales) / sp2.TotalSales) * 100 
        END AS SalesGrowthPercentage
    FROM SalesPerYear sp1
    LEFT JOIN SalesPerYear sp2 ON sp1.ProductID = sp2.ProductID AND sp1.Year = sp2.Year + 1
)
SELECT ProductID,Year,CurrentYearSales as total_sales,SalesGrowthPercentage
FROM SalesGrowth
